import { SnakeGameHistory, PlayerRanking } from '../types/snakeTypes';
import { formatGameTime, formatDate } from '../graph/init';

async function fetchSnakeRanking(): Promise<PlayerRanking[]> {
    try {
        // console.log('=== FETCHING SNAKE RANKING ===');
        const host = window.location.hostname;
        const port = window.location.port;
        const protocol = window.location.protocol;

        const response = await fetch(`${protocol}//${host}:${port}/api/snake/ranking`);
...
function generateRankingHTML(ranking: PlayerRanking[]): string {
    if (ranking.length === 0) {
        return `
            <tbody>
                <tr>
            ...
            ${ranking.map((player, index) => {
                ...
                return `
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="py-2">${player.maxSize}</td>
...
export async function updateRanking(): Promise<void> {
    try {
        const ranking = await fetchSnakeRanking();
