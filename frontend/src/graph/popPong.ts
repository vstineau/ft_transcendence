...
const popupHTML = `
	<div id="game-popup" class="fixed inset-0 bg-white/10 backdrop-blur flex items-center justify-center z-50" onclick="closeGameDetails(event)">
		<div class="${bgColor} rounded-xl p-4 max-w-xl h-[350px] w-full mx-4 transform transition-all duration-300 scale-95 relative flex flex-col justify-start ${textColor} ${shadowColor}" onclick="event.stopPropagation()">
			...
			<div class="flex items-center justify-between">
				<div class="text-center flex-1">
					...
					<div class="text-sm mt-1">
						<span class="${textColor}" data-translate="game.finalBallSpeed">Final ball speed :</span>
						<span class="${accentColor}">${game.finalBallSpeed || 0}</span>
					</div>
					<div class="text-sm">
						<span class="${textColor}" data-translate="game.averageSpeed">Average speed :</span>
						<span class="${accentColor}">${game.opponentStats?.finalBallSpeed || '-'}</span>
					</div>
				</div>
				...
				<div class="text-center flex-1">
					...
					<div class="text-sm">
						<span class="${textColor}" data-translate="game.averageSpeed">Average speed :</span>
						<span class="${accentColor}">${game.opponentStats ? Math.max((game.opponentStats.finalBallSpeed || 1) - 1, 0) : '-'}</span>
					</div>
				</div>
			</div>
		</div>
