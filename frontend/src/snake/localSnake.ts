...
class LocalSnakeGame {
    ...
    constructor() {
        this.WIN = Math.floor(window.innerHeight * 0.90);
    }
    ...
    private eatFood(snake: Snake, g: Game): boolean {
        ...
        const idx = g.foods.findIndex(f =>
            Math.abs(f.pos.x - head.x) < this.SEG_SIZE &&
            Math.abs(f.pos.y - head.y) < this.SEG_SIZE
        );
        if (idx !== -1) {
    ...
    private areSegmentsColliding(a: pos, b: pos): boolean {
        return (
            Math.abs(a.x - b.x) < (this.SEG_SIZE / 2) &&
            Math.abs(a.y - b.y) < (this.SEG_SIZE / 2)
        );
    }
    ...
    private resetGame(g: Game): void {
        g.p1.segments = [{ x: Math.floor(g.winSize * 0.25 / this.SEG_SIZE) * this.SEG_SIZE, y: Math.floor(g.winSize * 0.5 / this.SEG_SIZE) * this.SEG_SIZE }];
        g.p1.dir = { x: 0, y: -1 };
        g.p1.pendingDir = { x: 0, y: -1 };
        g.p2.segments = [{ x: Math.floor(g.winSize * 0.75 / this.SEG_SIZE) * this.SEG_SIZE, y: Math.floor(g.winSize * 0.5 / this.SEG_SIZE) * this.SEG_SIZE }];
        g.p2.dir = { x: 0, y: 1 };
    ...
    private initGame(): Game {
        const game: Game = {
            p1: {
                name: 'Player 1',
                segments: [{ x: Math.floor(this.WIN * 0.25 / this.SEG_SIZE) * this.SEG_SIZE, y: Math.floor(this.WIN * 0.5 / this.SEG_SIZE) * this.SEG_SIZE }],
                dir: { x: 0, y: -1 },
                pendingDir: { x: 0, y: -1 },
                color: "blue",
                id: 'p1'
            ...
            p2: {
                name: 'Player 2',
                segments: [{ x: Math.floor(this.WIN * 0.75 / this.SEG_SIZE) * this.SEG_SIZE, y: Math.floor(this.WIN * 0.5 / this.SEG_SIZE) * this.SEG_SIZE }],
                dir: { x: 0, y: 1 },
                pendingDir: { x: 0, y: 1 },
                color: "red",
                id: 'p2'
    ...
    private drawWinner(winner: Snake): void {
        ...
        this.ctx.textAlign = 'center';
        this.ctx.font = `40px Arial`;
        this.ctx.fillText(winner.name + ' wins!', this.canvas.width * 0.5, this.canvas.height * 0.33);
    }
