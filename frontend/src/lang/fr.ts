export default {
    "nav": {
        "dashboard": "Tableau de bord",
        "games": "Jeux",
        "settings": "Paramètres",
        "logout": "Déconnexion"
    },
    "dashboard": {
        "title": "FT_TRANSCENDENCE",
        "welcome": "Bienvenue",
        "back": "de retour",
        "games": "Jeux",
        "letsTalk": "Discutons",
        "lightMode": "Mode clair",
        "darkMode": "Mode sombre",
        "snakeStats": "Stats Snake",
        "pongStats": "Stats Pong",
        "yourMaxSize": "Votre taille max",
        "play": "Jouer",
        "view": "Voir",
        "edit": "Editer"
    },
    "games": {
        "pong": {
            "description": "Pong est l'un des premiers jeux informatiques jamais créés, le but est de vaincre votre adversaire."
        },
        "snake": {
            "description": "Pour Snake, vous devez empêcher le serpent de rentrer en collision avec les obstacles et lui-même."
        }
    },
    "welcome": {
        "subtitle": "Choisissez votre aventure.",
        "scroll": "défiler"
    },
    "auth": {
        "signIn": "Se connecter",
        "signUp": "S'inscrire",
        "createAccount": "Créer un compte",
        "welcomeBack": "Bon retour !",
        "login": "Identifiant",
        "password": "Mot de passe",
        "nickname": "Pseudo",
        "email": "Email",
        "profilePicture": "Photo de profil (optionnel)",
        "enable2fa": "Activer l'authentification à deux facteurs",
        "agreeTerms": "S'inscrire à Ft_transcendence signifie que vous acceptez",
        "privacyPolicy": "la Politique de confidentialité",
        "and": "et",
        "termsOfService": "les Conditions d'utilisation",
        "noAccount": "Vous n'avez pas de compte ?",
        "haveAccount": "Vous avez un compte ?"
    },
    "settings": {
        "profile": "Profil",
        "general": "Général",
        "profileSettings": "Paramètres du profil",
        "changePassword": "Changer le mot de passe",
        "dualAuth": "Authentification à deux facteurs",
        "profilePicture": "Photo de profil",
        "language": "Langue",
        "privacyTerms": "Politique de confidentialité générale<br>et conditions d'utilisation",
        "deleteAccount": "Supprimer votre compte",
        "backToDashboard": "← Retour au tableau de bord"
    },
};
