...
function isInvited(uidP1, uidP2, friend) {
...
function initRoom(socket, user, friend) {
...
function handleDisconnect(app, socket) {
    socket.on('disconnect', () => {
        ...
        if (room) {
            ...
            room.playersNb--;
            const sock = app.io.of('snake').sockets.get(room.game.p1.id === socket.id ? room.game.p2.id : room.game.p1.id);
            if (sock) {
...
function moveSnake(snake, winSize) {
...
function areSegmentsColliding(a, b) {
    return (Math.abs(a.x - b.x) < (SEG_SIZE / 2) &&
...
function checkCollision(snake, other) {
    ...
    if (otherBody.some(seg => areSegmentsColliding(seg, head)))
        return "other";
    if (areSegmentsColliding(head, otherHead))
        return "head-on";
    return null;
}
...
export function update(game, app, roomId) {
...
async function getUser(socket, cookie) {
    let user;
    if (cookie) {
...
export async function startSnakeGame(app) {
    app.io.of('/snake').on('connection', (socket) => {
        socket.on('isConnected', async (cookie, friend) => {
            ...
            socket.emit('roomJoined_snake', room.name);
