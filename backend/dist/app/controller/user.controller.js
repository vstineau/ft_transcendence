...
export const userController = (server, _opts, done) => {
    ...
    server.get('/logout', async (_request, reply) => {
        try {
    ...
    server.get('/deleteAccount', async (request, reply) => {
        try {
    ...
    server.get('/updateInfos', async (request, reply) => {
        try {
            if (request.cookies.token) {
                const payload = server.jwt.verify(request.cookies.token);
                const user = await User.findOneBy({ login: payload.login });
    ...
    server.post('/updateInfos', async (request, reply) => {
        try {
            if (request.cookies.token) {
                const payload = server.jwt.verify(request.cookies.token);
                const user = await User.findOneBy({ login: payload.login });
